...
# 普通的 GraphNode
def fetch_user_info_node(state: MessagesState, runtime) -> dict:
    # 获取 graph 上下文中的 messages
    msgs = state.get('messages', [])
    # 查看整个上下文是否已经存在用户的信息
    for msg in msgs:
        if isinstance(msg, AIMessage) and msg.id == 'user_info_success':
            return {}
...
# 总图构建器
graph_builder = StateGraph(MessagesState, context_schema=CtripContext)
# 添加图结点
